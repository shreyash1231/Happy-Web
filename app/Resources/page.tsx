...
import { fetchWordPressPosts, stripHtml } from "@/lib/wordpress";
    ...
    <>
      <div className="bg-[#E5DFD5] rounded-b-[60px] pb-10">
        ...
        <section className="pt-8 px-6 xl:p-8 mx-auto max-w-[1200px]">
          <h1 className="text-2xl md:text-4xl xl:text-5xl text-center text-[#544120]">
            Insights for a More Conscious Life
          </h1>
          ...
          <form action="/Resources" method="get" className="mt-8 max-w-2xl mx-auto flex gap-3">
            <input
              type="text"
              name="q"
              defaultValue={query}
            ...
            <button
              type="submit"
              className="rounded-full bg-[#3f5c4a] text-white px-6 py-3 hover:bg-[#162d22] transition"
            >
              Search
            </button>
          </form>
      ...
      <section className="px-6 xl:px-8 py-10 max-w-[1200px] mx-auto">
        {posts.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-[#8b7a5c] p-8 text-center text-[#544120]">
              ...
              return (
                <article key={post.id} className="rounded-3xl overflow-hidden bg-[#e9e1d6] shadow-sm">
                  ...
                  <div className="p-5 flex flex-col gap-4">
                    <h2 className="text-lg font-semibold text-[#544120] line-clamp-2">
                      {stripHtml(post.title?.rendered ?? "Untitled")}
                    </h2>
                    <p className="text-sm text-[#736345] line-clamp-3">{summary || "Read this article."}</p>
                    <Link
                      href={`/Resources/${post.slug}`}
                      className="text-[#3f5c4a] font-medium hover:underline"
                    >
                      Read Article →
                    </Link>
                  </div>
